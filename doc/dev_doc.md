...
│   │   ├── model_list.js             # 模型列表视图（卡片网格展示/下载/启动/停止）
...
**对应前端**：`views/model_list.js`（模型卡片网格、下载/启动/停止按钮）
...
响应式网格卡片布局（`repeat(auto-fill, minmax(320px, 1fr))`，窗口越宽一行卡片越多）：
...
├───────────────────────────────────────────────────────────────────┤
│ ┌─────────────────────────┐  ┌─────────────────────────┐          │
│ │ Qwen3.5-9B-Q4    [已启动]│  │ SD-XL           [可用]  │          │
│ │ 文本生成 · 5.6GB · 需内存 32GB│ 文本生成图片 · 6.9GB · ...│      │
│ │ [工具调用] [推理]        │  │                         │          │
│ │ ─────────────────────── │  │ ─────────────────────── │          │
│ │      [查看模型] [关闭模型]│  │      [生成图片] [删除]   │          │
│ │ ▓▓▓▓▓▓▓░░░ (下载进度条)  │  └─────────────────────────┘          │
...
1. **头部**：模型名（加粗、超长省略）+ 状态徽章（可用/不可用/已启动）右上角
2. **元信息行**：`模型类型 · 模型大小 · 需内存 X GB`（灰色小字）
3. **能力标签行**：仅展示支持的能力（`工具调用` / `推理` / `图片识别` 绿色胶囊标签），全不支持则该行隐藏
4. **操作按钮区**：分隔线 + 右对齐按钮（下载/启动/查看/关闭/删除），沿用原按钮样式
5. **底部进度条**：下载时在卡片底部显示 3px 紫色进度条（`updateProgressBar()` 按 `data-progress-bar` 属性更新宽度），主模型下载完成进入 mmproj/diffusion/vae 阶段时进度条归零重新走

卡片状态样式：

- **运行中**：左侧 3px 蓝色高亮边（`box-shadow: inset 3px 0 0 #2196f3`）+ 蓝色边框
- **不可用**（RAM-C 不足）：整卡透明度降至 0.6
- **hover**：边框变紫色 `#6c63ff` + 上浮 2px + 阴影
...
- **消息区手动/自动滚动模式（`agent.js`）**：流式输出期间 `renderMessages()` 频繁重建 DOM，原先会把用户点开的 `<details>`（推理过程/工具调用/工具结果）合上并强制滚到底部，导致无法点开查看。现在：
  - **手动模式**：鼠标进入 `#agent-msg-area`（`mouseenter`）即进入，重渲染时保留滚动位置（可上滑）；鼠标离开 1 秒后（`mouseleave` + 定时器）恢复自动模式并滚到底部。
  - **悬停提示**：鼠标在消息区停住 600ms（`mousemove` 防抖）后，消息区顶部居中淡入显示提示条「鼠标离开聊天区域，进入自动浏览模式」（`#agent-scroll-tip`，`pointer-events: none`），3 秒后自动消失，鼠标移动或离开时立即隐藏。
